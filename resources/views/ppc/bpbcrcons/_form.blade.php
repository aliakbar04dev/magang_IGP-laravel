<div class="box-body ">
	...
	<div class="form-group">
		<div class="col-sm-2{{ $errors->has('kd_line') ? ' has-error' : '' }}">
			...
			@else
			{!! Form::text('kd_line', null, ['class'=>'form-control','placeholder' => 'Line', 'readonly']) !!} 
			@endif
...
<div class="box-footer">
	...
	@endif
	&nbsp;&nbsp;
	<a class="btn btn-default" href="{{ route('bpbcrcons.index') }}">Cancel</a>
</div>

<!-- Popup Cust Modal -->
@include('ppc.bpbcrcons.popup.lineModal')
<!-- Popup Item Modal -->
@include('ppc.bpbcrcons.popup.itemModal')
	    ...
	    $("#btn-delete").click(function(){
	    	...
	    	if(no_doc !== "") {
	    		...
	    		swal({
	    			...
	    			var urlRedirect = "{{ route('bpbcrcons.destroy', 'param') }}";
	    	...
	    	if(index == null) {
	    		swal("Tidak ada data yang dipilih!", "", "warning");
				        ...
				        if(no_doc === '') {
				          ...
				          var url = "{{ route('bpbcrcons.hapus', ['param', 'param1'])}}";
	    	...
	    	swal({
	    		...
	    		focusCancel: true,
			            ...
			            var info3 = "warning";
				          ...
				          // delete via ajax
				          // hapus data detail dengan ajax
				          var url = "{{ route('bpbcrcons.hapusdetail', 'param')}}";
				          url = url.replace('param',  window.btoa(no_doc));
				          $.ajax({
				          	type     : 'POST',
				          	url      : url,
