  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{ route('hrdtkpis.approval') }}"><i class="fa fa-files-o"></i> KPI Division</a></li>
                        ...
                        @if ($hrdtkpi->hrdtKpiRejects()->get()->count() > 0)
                          ...
                          @foreach ($hrdtkpi->hrdtKpiRejects()->get() as $hrdtkpireject)
                            @if (Auth::user()->can(['hrd-kpi-approve','hrd-kpi-reject']))
                              {{ "&nbsp;-&nbsp;&nbsp;" }}<a target="_blank" href="{{ route('hrdtkpis.showapprovalrevisi', base64_encode($hrdtkpireject->id)) }}" data-toggle="tooltip" data-placement="top" title="Show History KPI Division No. Revisi {{ $hrdtkpireject->revisi }}">{{ $hrdtkpireject->revisi }}</a>
                            @else
          ...
          @if ($hrdtkpi->status === "SUBMIT")
            @if (Auth::user()->can(['hrd-kpi-approve','hrd-kpi-reject']))
              @if (Auth::user()->can('hrd-kpi-approve'))
                {!! Form::submit('Approve Superior', ['class'=>'btn btn-primary', 'id' => 'btn-save']) !!}
                &nbsp;&nbsp;
              @endif
              @if (Auth::user()->can('hrd-kpi-reject'))
                <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject Superior' onclick='reject("{{ base64_encode($hrdtkpi->id) }}")'>
                  <span class='glyphicon glyphicon-remove'></span> Reject Superior
                </button>
                &nbsp;&nbsp;
              @endif
            @endif
          @endif
          <a class="btn btn-primary" href="{{ route('hrdtkpis.approval') }}" data-toggle="tooltip" data-placement="top" title="Kembali ke Dashboard">Cancel</a>
        </div>
      {!! Form::close() !!}
    </section>
    <!-- /.content -->
    ...
    if (!localParams.send) {
      ...
      if(valid_data === "F") {
        ...
        swal({
          ...
          document.getElementById("st_input").value = "DIR";
  ...
  function reject(id)
  {
    ...
    swal({
      ...
      swal({
        ...
        urlRedirect = urlRedirect.replace('param3', window.btoa("DIR"));
