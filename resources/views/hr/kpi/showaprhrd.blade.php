  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{ route('hrdtkpis.approvalhrd') }}"><i class="fa fa-files-o"></i> KPI Division</a></li>
        <li class="active">Detail KPI Division</li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      {!! Form::model($hrdtkpi, ['url' => route('hrdtkpis.update', base64_encode($hrdtkpi->id)),
          ...
          <div class="col-md-12">
            <div class="box box-primary">
              <div class="box-header with-border">
                <h3 class="box-title">Detail KPI Division <b>{{ $hrdtkpi->desc_div." ".$hrdtkpi->tahun }}</b></h3>
              </div>
              <!-- /.box-header -->
              <div class="box-body">
                        ...
                        @if ($hrdtkpi->hrdtKpiRejects()->get()->count() > 0)
                          ...
                          @foreach ($hrdtkpi->hrdtKpiRejects()->get() as $hrdtkpireject)
                            @if (Auth::user()->can(['hrd-kpi-approvehrd','hrd-kpi-rejecthrd']))
                              {{ "&nbsp;-&nbsp;&nbsp;" }}<a target="_blank" href="{{ route('hrdtkpis.showapprovalhrdrevisi', base64_encode($hrdtkpireject->id)) }}" data-toggle="tooltip" data-placement="top" title="Show History KPI Division No. Revisi {{ $hrdtkpireject->revisi }}">{{ $hrdtkpireject->revisi }}</a>
                            @else
        ...
        <div class="box-footer">
          @if ($hrdtkpi->status === "APPROVE SUPERIOR")
            @if (Auth::user()->can(['hrd-kpi-approvehrd','hrd-kpi-rejecthrd']))
              ...
              @if (Auth::user()->can('hrd-kpi-rejecthrd'))
                <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject HRD' onclick='reject("{{ base64_encode($hrdtkpi->id) }}")'>
                  <span class='glyphicon glyphicon-remove'></span> Reject HRD
                </button>
                &nbsp;&nbsp;
              @endif
            @endif
          @elseif($hrdtkpi->status === "SUBMIT REVIEW Q1" || $hrdtkpi->status === "SUBMIT REVIEW Q2" || $hrdtkpi->status === "SUBMIT REVIEW Q3" || $hrdtkpi->status === "SUBMIT REVIEW Q4") 
            @if (Auth::user()->can(['hrd-kpi-approvehrd','hrd-kpi-rejecthrd']))
              @if (Auth::user()->can('hrd-kpi-approvehrd'))
                {!! Form::submit('Approve Review HRD', ['class'=>'btn btn-primary', 'id' => 'btn-save']) !!}
                &nbsp;&nbsp;
              @endif
              @if (Auth::user()->can('hrd-kpi-rejecthrd'))
                <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject Review HRD' onclick='reject("{{ base64_encode($hrdtkpi->id) }}")'>
                  <span class='glyphicon glyphicon-remove'></span> Reject Review HRD
                </button>
                &nbsp;&nbsp;
              @endif
            @endif
          @endif
          <a class="btn btn-primary" href="{{ route('hrdtkpis.approvalhrd') }}" data-toggle="tooltip" data-placement="top" title="Kembali ke Dashboard">Cancel</a>
        </div>
      {!! Form::close() !!}
    </section>
    <!-- /.content -->
...
<script type="text/javascript">
  ...
  $('#form_id').submit(function (e, params) {
    ...
    if (!localParams.send) {
      ...
      if(valid_data === "F") {
        ...
        var msg = "Anda yakin APPROVE KPI Division ini?";
  ...
  function reject(id)
  {
    var msg = "Anda yakin REJECT KPI Division ini?";
    var txt = "";
    //additional input validations can be done hear
    swal({
      title: msg,
      ...
      swal({
        ...
        var urlRedirect = "{{ route('hrdtkpis.reject', ['param', 'param2', 'param3']) }}";
