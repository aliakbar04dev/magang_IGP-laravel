<div class="box-body form-horizontal">
  ...
  <div class="form-group">
    <div class="col-sm-6 {{ $errors->has('npk_pic') ? ' has-error' : '' }}">
      ...
      {!! Form::select('npk_pic', \DB::table("mgmt_pics")->selectRaw("npk, npk||' - '||coalesce((select v.nama from v_mas_karyawan v where v.npk = mgmt_pics.npk limit 1),'-') nm_pic")->where('st_aktif', 'T')->where('st_dep', 'T')->orderBy('npk')->pluck('nm_pic','npk')->all(), null, ['class'=>'form-control select2', 'placeholder' => 'ALL']) !!}
...
<div class="box-footer">
  <button id='btnprint' type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Print Laporan Genba DEP' onclick='printLaporanGemba()'>
    <span class='glyphicon glyphicon-print'></span> Print Laporan Genba DEP
  </button>
  &nbsp;&nbsp;
  ...
  function printLaporanGemba()
  {
    ...
    if(tgl_awal == "" || tgl_akhir == "") {
      ...
      swal({
        ...
        var urlRedirect = "{{ route('mgmtgembadeps.printlaporan', ['param','param2','param3','param4']) }}";
