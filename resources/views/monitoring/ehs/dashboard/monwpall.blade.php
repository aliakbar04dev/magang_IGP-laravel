			...
			<div class="col-md-12">
				<div class="panel panel-default">
					<div class="panel-heading">
						<center><a href="{{ url('/monitoringwpall') }}"><h4>Monitoring Kontraktor IGP GROUP</h4></a></center>
					</div>
					...
					<div class="panel-body">
						<table id="tblMaster" class="table table-bordered" cellspacing="0" width="100%">
	    			        ...
	    			        <tbody>
	  			            	@foreach ($ehstwp1sIGP->get() as $data)
	  			            		<tr>
	    			        	...
	    			        	@endforeach
	    			        	@foreach ($ehstwp1sGKD->get() as $data)
	  			            		<tr>
  			            				@if($data->scan_sec_out_tgl == null && $data->skrg > $data->batas)
			    			        		<td style='text-align: center;color: white;' bgcolor="red">{{ $loop->iteration }}</td>
			    			        		<td style='color: white;' bgcolor="red">{{ $data->nm_supp }}</td>
	    			        	...
	    			        	@endforeach
	    			        	@foreach ($ehstwp1sAGI->get() as $data)
	  			            		<tr>
  			            				@if($data->scan_sec_out_tgl == null && $data->skrg > $data->batas)
			    			        		<td style='text-align: center;color: white;' bgcolor="red">{{ $loop->iteration }}</td>
			    			        		<td style='color: white;' bgcolor="red">{{ $data->nm_supp }}</td>
	...
	function changeTgl() {
		...
		if(var_tgl !== "") {
			...
			var tahun = date.getFullYear();
	      	...
	      	var urlRedirect = "{{ url('/monitoringwpall/param') }}";
	...
	setTimeout(function() {
		...
		if(var_tgl !== "") {
			...
			var tahun = date.getFullYear();
	      	...
	      	var urlRedirect = "{{ url('/monitoringwpall/param/param2') }}";
