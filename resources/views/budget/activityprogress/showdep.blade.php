...
@section('content')
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        CR Activities Progress [Dept]
        <small>Detail CR Activities Progress [Dept]</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="{{ url('/home') }}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><i class="fa fa-files-o"></i> Budget - TRANSAKSI</li>
        <li><a href="{{ route('bgttcrsubmits.indexdep') }}"><i class="fa fa-files-o"></i> CR Activities Progress [Dept]</a></li>
    ...
    <section class="content">
        ...
        <div class="col-md-12">
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">CR Activities Progress [Dept]</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                  ...
                  <tr>
                    ...
                    <td style="width: 25%;">
                      @if (Auth::user()->can('budget-cr-activities-approve-dep'))
                        <a target="_blank" href="{{ route('bgttcrregiss.showdep', base64_encode($bgttcrsubmit->id_regis)) }}" data-toggle="tooltip" data-placement="top" title="Show Detail Register">
      ...
      <div class="box-footer">
        @if ($bgttcrsubmit->checkKdDept() === "T" && $bgttcrsubmit->status === "SUBMIT")
          @if (Auth::user()->can('budget-cr-activities-approve-dep'))
            <button id='btnapprove' type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Approve Activities Progress - Dept. Head' onclick='approve()'>
              <span class='glyphicon glyphicon-check'></span> Approve Dept. Head
            </button>
            &nbsp;&nbsp;
            <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject Activities Progress - Dept. Head' onclick='reject()'>
              <span class='glyphicon glyphicon-remove'></span> Reject Dept. Head
            </button>
            &nbsp;&nbsp;
          @endif
        @endif
        <a class="btn btn-primary" href="{{ route('bgttcrsubmits.indexdep') }}" data-toggle="tooltip" data-placement="top" title="Kembali ke Daftar CR Activities Progress">Cancel</a>
      </div>
    </section>
    <!-- /.content -->
  </div>
...
<script type="text/javascript">
  ...
  function approve()
  {
    ...
    swal({
      ...
      var url = "{{ route('bgttcrsubmits.approvedep')}}";
      $("#loading").show();
      $.ajax({
        type     : 'POST',
        url      : url,
        ...
        data     : {
          ...
          _token         : token,
          ids            : "{{ $bgttcrsubmit->id }}",
          status_approve : window.btoa("DEP")
        },
        success:function(data){
          $("#loading").hide();
          if(data.status === 'OK'){
            swal("Approved", data.message, "success");
            var urlRedirect = "{{ route('bgttcrsubmits.showdep', 'param') }}";
  ...
  function reject()
  {
    ...
    swal({
      ...
      swal({
        ...
        // save via ajax
        // create data detail dengan ajax
        var url = "{{ route('bgttcrsubmits.rejectdep')}}";
        $("#loading").show();
        $.ajax({
          type     : 'POST',
          url      : url,
