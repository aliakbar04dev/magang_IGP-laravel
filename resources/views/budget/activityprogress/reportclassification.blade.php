  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        ...
        <small>CR Classification Report</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="{{ url('/home') }}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><i class="fa fa-files-o"></i> Budget - REPORT</li>
        <li class="active"><i class="fa fa-files-o"></i> CR Classification Report</li>
      </ol>
    </section>
        ...
        <div class="col-md-12">
          <div class="box box-primary">
            <div class="box-body form-horizontal">
              ...
              <div class="form-group">
                <div class="col-sm-6">
                  {!! Form::label('lblclassification', 'Classification') !!}
                  <select name="filter_nm_klasifikasi" aria-controls="filter_status" class="form-control select2" multiple="multiple">
                    @foreach($bgtt_cr_klasifis->get() as $bgtt_cr_klasifi)
                      <option value="{{$bgtt_cr_klasifi->nm_klasifikasi}}">{{$bgtt_cr_klasifi->nm_klasifikasi}}</option>
                    @endforeach
                  </select>
                </div>
                <div class="col-sm-2">
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    var tableMaster = $('#tblMaster').DataTable({
      ...
      // serverSide: true,
      ajax: "{{ route('bgttcrsubmits.dashboardreportclassification') }}",
      columns: [
      	{data: null, name: null},
    ...
    $('#tblMaster tbody').on( 'click', 'tr', function () {
      if ($(this).hasClass('selected') ) {
        ...
        if(tableMaster.rows().count() > 0) {
          ...
          var nm_klasifikasi = tableMaster.cell(index, 1).data();
          var info = "Classification: " + nm_klasifikasi + ", Tahun: " + tahun + ", Bulan: " + bulan;
          document.getElementById("info-detail").innerHTML = 'Detail (' + info + ')';
          var regex = /(<([^>]+)>)/ig;
          initTable(window.btoa(tahun.replace(regex, "")), window.btoa(bulan.replace(regex, "")), window.btoa(nm_klasifikasi.replace(regex, "")));
        }
      }
    });

    var url = '{{ route('bgttcrsubmits.dashboardreportclassificationdetail', ['param', 'param2', 'param3']) }}';
    ...
    function initTable(tahun, bulan, nm_klasifikasi) {
      tableDetail.search('').columns().search('').draw();
      var url = '{{ route('bgttcrsubmits.dashboardreportclassificationdetail', ['param', 'param2', 'param3']) }}';
      url = url.replace('param3', nm_klasifikasi);
    ...
    $("#tblMaster").on('preXhr.dt', function(e, settings, data) {
      ...
      data.nm_klasifikasi = $('select[name="filter_nm_klasifikasi"]').val();
