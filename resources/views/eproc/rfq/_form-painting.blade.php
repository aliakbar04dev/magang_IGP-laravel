...
<div class="box-body">
  <div class="row form-group">
    <div class="col-sm-3 {{ $errors->has('part_weight_kg') ? ' has-error' : '' }}">
    ...
    <div class="col-sm-3 {{ $errors->has('surf_area_mm') ? ' has-error' : '' }}">
      {!! Form::label('surf_area_mm', 'Surface Area (mm2)') !!}
      {!! Form::number('surf_area_mm', !empty($prctrfq->surf_area_mm) ? numberFormatterForm(0, 2)->format($prctrfq->surf_area_mm) : null, ['class'=>'form-control', 'placeholder' => 'Surface Area (mm2)', 'max' => 9999999999.99, 'step' => 'any', 'id' => 'surf_area_mm', 'onchange' => 'refreshTotal()', 'style' => 'text-align:right;']) !!}
      {!! $errors->first('surf_area_mm', '<p class="help-block">:message</p>') !!}
    </div>
  </div>
  <!-- /.form-group -->
</div>
...
<ul class="nav nav-tabs" role="tablist">
  <li role="presentation" class="active" id="nav_rm">
    <a href="#rm" aria-controls="rm" role="tab" data-toggle="tab" title="1. Chemical Composition Cost">
      1. Chemical Composition Cost
    </a>
  </li>
...
<div class="tab-content">
  <div role="tabpanel" class="tab-pane active" id="rm">
    <div class="box-body" id="field-rm">
      @foreach ($prctrfq->prctRfqRms() as $model)
        <div class="row form-group" id="rm_field_{{ $loop->iteration }}">
          ...
          <div class="col-sm-5">
            <label name="rm_mat_chemical_{{ $loop->iteration }}">Chemical Name (*)</label>
            <input type="text" id="rm_mat_chemical_{{ $loop->iteration }}" name="rm_mat_chemical_{{ $loop->iteration }}" required class="form-control" placeholder="Chemical Name" maxlength="50" value="{{ $model->mat_chemical }}">
          </div>
          <div class="col-sm-4">
            <label name="rm_mat_supplier_{{ $loop->iteration }}">Supplier Name</label>
            <input type="text" id="rm_mat_supplier_{{ $loop->iteration }}" name="rm_mat_supplier_{{ $loop->iteration }}" class="form-control" placeholder="Supplier Name" maxlength="50" value="{{ $model->mat_supplier }}">
          </div>
          <div class="col-sm-1">
            <label name="rm_btndelete_{{ $loop->iteration }}">Action</label>
            <button id="rm_btndelete_{{ $loop->iteration }}" name="rm_btndelete_{{ $loop->iteration }}" type="button" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="top" title="Hapus Chemical Composition Cost" onclick="deleteRm(this)">
        ...
        <div class="row form-group" id="rm_field2_{{ $loop->iteration }}">
          <div class="col-sm-4">
            <label name="rm_mat_spec_{{ $loop->iteration }}">Specification</label>
            <input type="text" id="rm_mat_spec_{{ $loop->iteration }}" name="rm_mat_spec_{{ $loop->iteration }}" class="form-control" placeholder="Specification" maxlength="50" value="{{ $model->mat_spec }}">
          </div>
          <div class="col-sm-2">
            <label name="rm_mat_consump_kg_{{ $loop->iteration }}">Consumption (Kg)</label>
            <input type="number" id="rm_mat_consump_kg_{{ $loop->iteration }}" name="rm_mat_consump_kg_{{ $loop->iteration }}" class="form-control" placeholder="Consumption (Kg)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;" value="{{ numberFormatterForm(0, 2)->format($model->mat_consump_kg) }}">
          </div>
          <div class="col-sm-3">
          ...
          <div class="col-sm-3">
            ...
            <input type="text" id="rm_ttl_mat_{{ $loop->iteration }}" name="rm_ttl_mat_{{ $loop->iteration }}" class="form-control" placeholder="Total (IDR)" disabled="" style="text-align:right;" value="{{ numberFormatter(0, 2)->format($model->ttl_mat) }}">
          </div>
        </div>
    ...
    <div class="box-body">
      <p class="pull-right">
      <button id="addRowRm" type="button" class="btn btn-success" data-toggle="tooltip" data-placement="top" title="Add Chemical Composition Cost"><span class="glyphicon glyphicon-plus"></span> Add Chemical Composition Cost</button>
      </p>
    </div>
    <!-- /.box-body -->
    <div class="box-body">
    ...
    <div class="box-body" id="field-proses">
      @foreach ($prctrfq->prctRfqProsess() as $model)
        <div class="row form-group" id="proses_field_{{ $loop->iteration }}">
          <div class="col-sm-1">
            <label name="proses_no_urut_{{ $loop->iteration }}">Urut (*)</label>
          ...
          <div class="col-sm-3">
            ...
            <input type="text" id="proses_nm_proses_{{ $loop->iteration }}" name="proses_nm_proses_{{ $loop->iteration }}" required class="form-control" placeholder="Process Name" maxlength="50" value="{{ $model->nm_proses }}">
          </div>
          <div class="col-sm-2">
            <label name="proses_ct_proses_dtk_{{ $loop->iteration }}">Cycle Time (sec)</label>
            <input type="number" id="proses_ct_proses_dtk_{{ $loop->iteration }}" name="proses_ct_proses_dtk_{{ $loop->iteration }}" class="form-control" placeholder="Cycle Time (sec)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;" value="{{ numberFormatterForm(0, 2)->format($model->ct_proses_dtk) }}">
          </div>
          <div class="col-sm-2">
            <label name="proses_pric_pros_idr_{{ $loop->iteration }}">Rate/sec (IDR)</label>
            <input type="number" id="proses_pric_pros_idr_{{ $loop->iteration }}" name="proses_pric_pros_idr_{{ $loop->iteration }}" class="form-control" placeholder="Rate/sec (IDR)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;" value="{{ numberFormatterForm(0, 2)->format($model->pric_pros_idr) }}">
          </div>
          <div class="col-sm-3">
...
<script type="text/javascript">
  ...
  function refreshTab() {
    ...
    if(st_rm === "T") {
      ...
      document.getElementById("part_weight_kg").focus();
    } else if(st_proses === "T") {
  ...
  function refreshTotal() {
    ...
    for($i = 1; $i <= jml_row_rm; $i++) {
      var mat_consump_kg = document.getElementById("rm_mat_consump_kg_" + $i).value.trim();
      mat_consump_kg = Number(mat_consump_kg);
      var pric_per_kg_idr = document.getElementById("rm_pric_per_kg_idr_" + $i).value.trim();
      pric_per_kg_idr = Number(pric_per_kg_idr);
      
      var ttl_mat = mat_consump_kg * pric_per_kg_idr;
  ...
  $("#addRowRm").click(function(){
    ...
    var rm_mat_chemical = 'rm_mat_chemical_'+jml_row_rm;
    var rm_mat_supplier = 'rm_mat_supplier_'+jml_row_rm;
    var rm_mat_spec = 'rm_mat_spec_'+jml_row_rm;
    var rm_mat_consump_kg = 'rm_mat_consump_kg_'+jml_row_rm;
    var rm_pric_per_kg_idr = 'rm_pric_per_kg_idr_'+jml_row_rm;
    var rm_ttl_mat = 'rm_ttl_mat_'+jml_row_rm;
    ...
    $("#field-rm").append(
      '<div class="row form-group" id="' + rm_id_field + '">\
        ...
        <div class="col-sm-5">\
          <label name="' + rm_mat_chemical + '">Chemical Name (*)</label>\
          <input type="text" id="' + rm_mat_chemical + '" name="' + rm_mat_chemical + '" required class="form-control" placeholder="Chemical Name" maxlength="50">\
        </div>\
        <div class="col-sm-4">\
          <label name="' + rm_mat_supplier + '">Supplier Name</label>\
          <input type="text" id="' + rm_mat_supplier + '" name="' + rm_mat_supplier + '" class="form-control" placeholder="Supplier Name" maxlength="50">\
        </div>\
        <div class="col-sm-1">\
          <label name="' + rm_btndelete + '">Action</label>\
          <button id="' + rm_btndelete + '" name="' + rm_btndelete + '" type="button" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="top" title="Hapus Chemical Composition Cost" onclick="deleteRm(this)">\
            <i class="fa fa-times"></i>\
          </button>\
        </div>\
      </div>\
      <div class="row form-group" id="' + rm_id_field2 + '">\
        <div class="col-sm-4">\
          <label name="' + rm_mat_spec + '">Specification</label>\
          <input type="text" id="' + rm_mat_spec + '" name="' + rm_mat_spec + '" class="form-control" placeholder="Specification" maxlength="50">\
        </div>\
        <div class="col-sm-2">\
          <label name="' + rm_mat_consump_kg + '">Consumption (Kg)</label>\
          <input type="number" id="' + rm_mat_consump_kg + '" name="' + rm_mat_consump_kg + '" class="form-control" placeholder="Consumption (Kg)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;">\
        </div>\
        <div class="col-sm-3">\
        ...
        <div class="col-sm-3">\
          <label name="' + rm_ttl_mat + '">Total (IDR)</label>\
          <input type="text" id="' + rm_ttl_mat + '" name="' + rm_ttl_mat + '" class="form-control" placeholder="Total (IDR)" disabled="" style="text-align:right;">\
        </div>\
      </div>\
      <HR id="' + rm_id_hr + '">'
    );
  ...
  function changeIdRm(row) {
    ...
    for($i = nextRow; $i <= jml_row_rm; $i++) {
      ...
      var id_mat_chemical = "#rm_mat_chemical_" + $i;
      var id_mat_chemical_new = "rm_mat_chemical_" + ($i-1);
      $(id_mat_chemical).attr({"id":id_mat_chemical_new, "name":id_mat_chemical_new});
      var id_mat_supplier = "#rm_mat_supplier_" + $i;
      var id_mat_supplier_new = "rm_mat_supplier_" + ($i-1);
      $(id_mat_supplier).attr({"id":id_mat_supplier_new, "name":id_mat_supplier_new});
      ...
      var id_mat_consump_kg = "#rm_mat_consump_kg_" + $i;
      var id_mat_consump_kg_new = "rm_mat_consump_kg_" + ($i-1);
      $(id_mat_consump_kg).attr({"id":id_mat_consump_kg_new, "name":id_mat_consump_kg_new});
      ...
      var id_ttl_mat = "#rm_ttl_mat_" + $i;
      var id_ttl_mat_new = "rm_ttl_mat_" + ($i-1);
      $(id_ttl_mat).attr({"id":id_ttl_mat_new, "name":id_ttl_mat_new});
  ...
  function deleteRm(ths) {
    var msg = 'Anda yakin menghapus Chemical Composition Cost ini?';
    swal({
      title: msg,
      text: "",
      type: 'warning',
  ...
  $("#addRowProses").click(function(){
    ...
    $("#field-proses").append(
      '<div class="row form-group" id="' + proses_id_field + '">\
        <div class="col-sm-1">\
          <label name="' + proses_no_urut + '">Urut (*)</label>\
        ...
        <div class="col-sm-3">\
          <label name="' + proses_nm_proses + '">Process Name (*)</label>\
          <input type="text" id="' + proses_nm_proses + '" name="' + proses_nm_proses + '" required class="form-control" placeholder="Process Name" maxlength="50">\
        </div>\
        <div class="col-sm-2">\
          <label name="' + proses_ct_proses_dtk + '">Cycle Time (sec)</label>\
          <input type="number" id="' + proses_ct_proses_dtk + '" name="' + proses_ct_proses_dtk + '" class="form-control" placeholder="Cycle Time (sec)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;">\
        </div>\
        <div class="col-sm-2">\
          <label name="' + proses_pric_pros_idr + '">Rate/sec (IDR)</label>\
          <input type="number" id="' + proses_pric_pros_idr + '" name="' + proses_pric_pros_idr + '" class="form-control" placeholder="Rate/sec (IDR)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;">\
        </div>\
        <div class="col-sm-3">\
