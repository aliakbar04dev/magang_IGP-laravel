...
<div class="box-body">
  <div class="row form-group">
    ...
    <div class="col-sm-4 {{ $errors->has('mat_price_period') ? ' has-error' : '' }}">
      ...
      {!! Form::text('mat_price_period', null, ['class' => 'form-control', 'placeholder' => 'Material Price Period', 'maxlength' => 20, 'id' => 'mat_price_period']) !!}
    ...
    <div class="col-sm-2 {{ $errors->has('casting_weight') ? ' has-error' : '' }}">
      ...
      {!! Form::number('casting_weight', !empty($prctrfq->casting_weight) ? numberFormatterForm(0, 2)->format($prctrfq->casting_weight) : null, ['class'=>'form-control', 'placeholder' => 'Casting Weight (Kg)', 'max' => 9999999999.99, 'step' => 'any', 'id' => 'casting_weight', 'onchange' => 'refreshTotal()', 'style' => 'text-align:right;']) !!}
      ...
      @foreach ($prctrfq->prctRfqRms() as $model)
        <div class="row form-group" id="rm_field_{{ $loop->iteration }}">
          ...
          <div class="col-sm-3">
            ...
            <input type="text" id="rm_mat_spec_{{ $loop->iteration }}" name="rm_mat_spec_{{ $loop->iteration }}" required class="form-control" placeholder="Material Spec" maxlength="50" value="{{ $model->mat_spec }}">
          </div>
          <div class="col-sm-2">
            <label name="rm_usage_kg_{{ $loop->iteration }}">Usage (Kg)</label>
            <input type="number" id="rm_usage_kg_{{ $loop->iteration }}" name="rm_usage_kg_{{ $loop->iteration }}" class="form-control" placeholder="Usage (Kg)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;" value="{{ numberFormatterForm(0, 2)->format($model->usage_kg) }}">
          </div>
          <div class="col-sm-2">
      ...
      @foreach ($prctrfq->prctRfqProsess() as $model)
        <div class="row form-group" id="proses_field_{{ $loop->iteration }}">
          ...
          <div class="col-sm-3">
            ...
            <input type="number" id="proses_ttl_proses_{{ $loop->iteration }}" name="proses_ttl_proses_{{ $loop->iteration }}" class="form-control" placeholder="Total (IDR)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;" value="{{ numberFormatterForm(0, 2)->format($model->ttl_proses) }}">
          </div>
          <div class="col-sm-1">
      ...
      @foreach ($prctrfq->prctRfqTools() as $model)
        <div class="row form-group" id="tool_field_{{ $loop->iteration }}">
          <div class="col-sm-1">
            ...
            <input type="number" id="tool_no_urut_{{ $loop->iteration }}" name="tool_no_urut_{{ $loop->iteration }}" required class="form-control" placeholder="Urut" min="1" max="9999999999" value="{{ $model->no_urut }}">
...
<script type="text/javascript">
  ...
  $("#addRowRm").click(function(){
    ...
    var rm_usage_kg = 'rm_usage_kg_'+jml_row_rm;
    ...
    $("#field-rm").append(
      '<div class="row form-group" id="' + rm_id_field + '">\
        <div class="col-sm-1">\
          <label name="' + rm_no_urut + '">Urut (*)</label>\
        ...
        <div class="col-sm-3">\
          <label name="' + rm_mat_spec + '">Material Spec (*)</label>\
          <input type="text" id="' + rm_mat_spec + '" name="' + rm_mat_spec + '" required class="form-control" placeholder="Material Spec" maxlength="50">\
        </div>\
        <div class="col-sm-2">\
          <label name="' + rm_usage_kg + '">Usage (Kg)</label>\
          <input type="number" id="' + rm_usage_kg + '" name="' + rm_usage_kg + '" class="form-control" placeholder="Usage (Kg)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;">\
        </div>\
        <div class="col-sm-2">\
  ...
  function changeIdRm(row) {
    ...
    for($i = nextRow; $i <= jml_row_rm; $i++) {
      ...
      var id_usage_kg = "#rm_usage_kg_" + $i;
      var id_usage_kg_new = "rm_usage_kg_" + ($i-1);
      $(id_usage_kg).attr({"id":id_usage_kg_new, "name":id_usage_kg_new});
