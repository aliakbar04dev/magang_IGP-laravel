...
<p>
	Kepada: <strong>{{ $kpd }} Laporan Pekerjaan</strong>
</p>
@else
<p>
...
<p>
	Telah disetujui Laporan Pekerjaan dengan No: <strong>{{ $tmtcwo1->no_wo }}</strong> oleh {{ $oleh }}: <strong>{{ Auth::user()->name }} ({{ Auth::user()->username }})</strong> dengan detail sebagai berikut:
</p>
...
<p>
	...
	@if ($tmtcwo1->st_main_item === "T")
		- Main Item: YA.<br>
		- IC: {{ $tmtcwo1->no_ic }} - {{ $tmtcwo1->nm_ic }}.
	@else 
		- Main Item: TIDAK.
	@endif
	@if (!empty($tmtcwo1->no_lhp))
		...
		- LS Mulai: {{ \Carbon\Carbon::parse($tmtcwo1->ls_mulai)->format('d/m/Y H:i') }}.
	@endif
</p>
