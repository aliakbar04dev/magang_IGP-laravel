  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{ route('picas.all') }}"><i class="fa fa-exchange"></i> PICA</a></li>
    ...
    <section class="content">
              ...
              <table class="table table-striped" cellspacing="0" width="100%">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td style="width: 20%;">
                      @if (Auth::user()->can(['pica-view','pica-approve','pica-reject']))
                        <a href="{{ route('qprs.showall', base64_encode($pica->issue_no)) }}" data-toggle="tooltip" data-placement="top" title="Show Detail QPR {{ $pica->issue_no }}">{{ $pica->issue_no }}</a>
                      @else
                        {{ $pica->issue_no }}
                      @endif
                    </td>
                  ...
                  <tr>
                    ...
                    <td style="width: 1%;"><b>:</b></td>
                    <td style="width: 20%;">{{ $pica->status }}</td>
                    <td style="width: 10%;"><b>Supplier</b></td>
                    <td style="width: 1%;"><b>:</b></td>
                    <td>{{ $pica->kd_supp." - ".$pica->namaSupp($pica->kd_supp) }}</td>
                  </tr>
                  ...
                  @if (!empty($pica->reject_tgl))
                    ...
                    <tr>
                      ...
                      <td style="width: 1%;"><b>:</b></td>
                      <td colspan="4">{{ $pica->status_reject." - ".$pica->reject_ket }}</td>
                    </tr>
                  @endif
      ...
      @if ($pica->status === 'SUBMIT')
        @if (Auth::user()->can('pica-approve'))
          <button id='btnapprove' type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Approve PICA {{ $pica->no_pica }}' onclick='approvePica("{{ base64_encode($pica->id) }}", "{{ $pica->no_pica }}")'>
            <span class='glyphicon glyphicon-check'></span> Approve PICA
          </button>
          &nbsp;&nbsp;
        @endif
        @if (Auth::user()->can('pica-reject'))
          <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject PICA {{ $pica->no_pica }}' onclick='rejectPica("{{ base64_encode($pica->id) }}", "{{ $pica->no_pica }}")'>
            <span class='glyphicon glyphicon-remove'></span> Reject PICA
          </button>
          &nbsp;&nbsp;
        @endif
      @elseif ($pica->status === 'APPROVE')
        @if (Auth::user()->can('pica-approve'))
          <button id='btnclose' type='button' class='btn bg-black' data-toggle='tooltip' data-placement='top' title='Close PICA {{ $pica->no_pica }}' onclick='closePica("{{ base64_encode($pica->id) }}", "{{ $pica->no_pica }}")'>
            <span class='glyphicon glyphicon-eye-close'></span> Close PICA
          </button>
          &nbsp;&nbsp;
        @endif
      @elseif ($pica->status === 'CLOSE')
        @if (Auth::user()->can('pica-approve'))
          <button id='btnefektif' type='button' class='btn bg-purple' data-toggle='tooltip' data-placement='top' title='Efektif PICA {{ $pica->no_pica }}' onclick='efektifPica("{{ base64_encode($pica->id) }}", "{{ $pica->no_pica }}")'>
            <span class='glyphicon glyphicon-thumbs-up'></span> Efektif PICA
          </button>
          &nbsp;&nbsp;
        @endif
      @endif
      ...
      &nbsp;&nbsp;
      <a class="btn btn-primary" href="{{ route('picas.all') }}" data-toggle="tooltip" data-placement="top" title="Kembali ke Dashboard PICA">Cancel</a>
      </div>
    </section>
    <!-- /.content -->
  </div>
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          if(data.status === 'OK'){
            ...
            var urlRedirect = "{{ route('picas.showall', 'param') }}";
