...
class ApprovalPklModel extends Model
{
    // public function Koneksi()
    // {
    //     $konek = DB::connection('oracle-usrintra')
    //         ->table("usrhrcorp.tpkla")
    //         ->select('tpkla.*')
    //         // ->whereYear('tpkla.tgl_pkl', '2018')
    //         // ->whereMonth('tpkla.tgl_pkl', '08')
    //         ->where('tpkla.tgl_pkl', '2018-08-26 00:00:00')
    //         ->get();
    ...
    public function namaByNpk($npk)
    {
        $nama = DB::table("v_mas_karyawan")
            ...
            ->value("nama");
        if($nama == null) {
            $nama = DB::connection('oracle-usrintra')
                ->table("usrhrcorp.mas_karyawan")
                ->select("nama")
                ->where("npk", "=", $npk)
                ->value("nama");
        }
