...
<p>
	Telah disetujui Register PP bagian {{ $ppReg->nm_dept }} ({{ $ppReg->kd_dept_pembuat }}) oleh Purchasing ({{ Auth::user()->name }} - {{ Auth::user()->username }}) dengan detail sebagai berikut:
</p>
...
<p>
	...
	- Supplier: {{ $ppReg->namaSupp($ppReg->kd_supp) }} ({{ $ppReg->kd_supp }}).<br>
</p>
