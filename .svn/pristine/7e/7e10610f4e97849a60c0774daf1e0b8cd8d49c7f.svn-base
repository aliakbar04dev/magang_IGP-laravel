...
<p>
	Kepada: Bapak/Ibu <strong>{{ $hrdtidp1->namaByNpk($hrdtidp1->npk_dep_head) }}</strong>
</p>

<p>
	Mohon maaf, Individual Development Plan Section Head telah ditolak oleh: {{ Auth::user()->name }} ({{ Auth::user()->username }}) pada tanggal {{ \Carbon\Carbon::now()->format('d M Y H:i') }} dengan detail sebagai berikut:
</p>
