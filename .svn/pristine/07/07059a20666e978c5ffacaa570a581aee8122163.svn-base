<div class="box-body form-horizontal">
  <div class="form-group">
    <div class="col-sm-3 {{ $errors->has('tgl_awal') ? ' has-error' : '' }}">
    ...
    <div class="col-sm-3 {{ $errors->has('tgl_akhir') ? ' has-error' : '' }}">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-3 {{ $errors->has('st_gemba') ? ' has-error' : '' }}">
      {!! Form::label('st_gemba', 'Close') !!}
      {!! Form::select('st_gemba', ['ALL' => 'ALL', 'T' => 'YES', 'F' => 'NO'], null, ['class'=>'form-control select2']) !!}
  ...
  <div class="form-group">
    <div class="col-sm-6 {{ $errors->has('npk_pic') ? ' has-error' : '' }}">
      ...
      {!! Form::select('npk_pic', \DB::table("mgmt_pics")->selectRaw("npk, npk||' - '||coalesce((select v.nama from v_mas_karyawan v where v.npk = mgmt_pics.npk limit 1),'-') nm_pic")->where('st_aktif', 'T')->where('st_ehs', 'T')->orderBy('npk')->pluck('nm_pic','npk')->all(), null, ['class'=>'form-control select2', 'placeholder' => 'ALL']) !!}
...
<div class="box-footer">
  <button id='btnprint' type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Print Laporan EHS Patrol' onclick='printLaporanGemba()'>
    <span class='glyphicon glyphicon-print'></span> Print Laporan EHS Patrol
  </button>
  &nbsp;&nbsp;
  ...
  function printLaporanGemba()
  {
    ...
    if(tgl_awal == "" || tgl_akhir == "") {
      ...
      swal({
        ...
        var st_gemba = document.getElementById("st_gemba").value;
        if(st_gemba == "") {
          st_gemba = "ALL";
        }
        ...
        var urlRedirect = "{{ route('mgmtgembaehss.printlaporan', ['param','param2','param3','param4','param5']) }}";
        urlRedirect = urlRedirect.replace('param5', window.btoa(npk_pic));
        urlRedirect = urlRedirect.replace('param4', window.btoa(st_gemba));
