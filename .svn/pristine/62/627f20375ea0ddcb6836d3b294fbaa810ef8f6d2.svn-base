  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <div class="form-group{{ $errors->has('kd_dep') ? ' has-error' : '' }}">
        {!! Form::label('kd_dep', 'Departemen (*)') !!}
        @if (empty($prcmnpk->npk))
          {!! Form::select('kd_dep[]', ['B' => 'BUYER', 'G' => 'GENERAL', 'P' => 'PROJECT'], null, ['class'=>'form-control select2','multiple'=>'multiple','data-placeholder' => 'Pilih Departemen', 'required', 'id' => 'kd_dep']) !!}
        @else
          {!! Form::select('kd_dep[]', ['B' => 'BUYER', 'G' => 'GENERAL', 'P' => 'PROJECT'], $deps, ['class'=>'form-control select2','multiple'=>'multiple','data-placeholder' => 'Pilih Departemen', 'required', 'id' => 'kd_dep']) !!}
        @endif
...
<div class="box-footer">
  ...
  @if (!empty($prcmnpk->npk))
    ...
    &nbsp;&nbsp;
  @endif
  <a class="btn btn-default" href="{{ route('prcmnpks.index') }}">Cancel</a>
    &nbsp;&nbsp;<p class="help-block pull-right has-error">{!! Form::label('info', '(*) tidak boleh kosong', ['style'=>'color:red']) !!}</p>
</div>

<!-- Modal Karyawan -->
@include('eproc.po.npk.popup.karyawanModal')
...
<script type="text/javascript">
  ...
  function keyPressed(e) {
    if(e.keyCode == 120) { //F9
      ...
      e.preventDefault();
      document.getElementById('kd_dep').focus();
    }
  }
  ...
  function popupKaryawan() {
    ...
    var url = '{{ route('datatables.popupKaryawanDeps') }}';
  ...
  function validateKaryawan() {
    var npk = document.getElementById('npk').value.trim();
    if(npk !== '') {
      var url = '{{ route('datatables.validasiKaryawanDep', 'param') }}';
