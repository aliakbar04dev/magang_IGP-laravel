...
<div class="box-body">
    ...
    <div class="col-sm-5 {{ $errors->has('pict_gemba') ? ' has-error' : '' }}">
      ...
      @else 
        ...
        @if (!empty($mgmtgemba->pict_gemba))
          <p>
            <img src="{{ $mgmtgemba->pictGemba() }}" alt="File Not Found" class="img-rounded img-responsive" style="border: 1px solid #ddd;border-radius: 4px;padding: 5px;width: 100px;">
            <a class="btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus File" href="{{ route('mgmtgembas.deleteimage', [base64_encode($mgmtgemba->no_gemba), base64_encode("GEMBA")]) }}"><span class="glyphicon glyphicon-remove"></span></a>
          </p>
        @endif
      @endif
  ...
  <div class="row form-group">
    <div class="col-sm-5 {{ $errors->has('npk_pic') ? ' has-error' : '' }}">
      ...
      @if ($mode_cm === "T")
        {!! Form::text('initial', $mgmtgemba->npk_pic." - ".$mgmtgemba->initial, ['class'=>'form-control','placeholder' => 'PIC Genba', 'disabled'=>'', 'id' => 'initial']) !!}
        {!! Form::hidden('npk_pic', null, ['class'=>'form-control','placeholder' => 'PIC Genba', 'required', 'id' => 'npk_pic', 'readonly'=>'readonly']) !!}
      @else 
        @if (!empty($mgmtgemba->kd_site))
          @if ($mgmtgemba->kd_site === "IGPJ")
            {!! Form::select('npk_pic', \DB::table("mgmt_pics")->selectRaw("npk, npk||' - '||coalesce((select coalesce(v.initial, split_part(v.nama,' ',1)) from v_mas_karyawan v where v.npk = mgmt_pics.npk limit 1),'-') initial")->where('st_aktif', 'T')->where('st_bod', 'T')->where('kd_site', 'IGPJ')->orderBy('npk')->pluck('initial','npk')->all(), null, ['class'=>'form-control select2', 'placeholder' => 'Pilih PIC Genba', 'required']) !!}
          @else 
            {!! Form::select('npk_pic', \DB::table("mgmt_pics")->selectRaw("npk, npk||' - '||coalesce((select coalesce(v.initial, split_part(v.nama,' ',1)) from v_mas_karyawan v where v.npk = mgmt_pics.npk limit 1),'-') initial")->where('st_aktif', 'T')->where('st_bod', 'T')->where('kd_site', 'IGPK')->orderBy('npk')->pluck('initial','npk')->all(), null, ['class'=>'form-control select2', 'placeholder' => 'Pilih PIC Genba', 'required']) !!}
          @endif
        @else 
          @if (Auth::user()->masKaryawan()->kode_site === "IGPJ")
            {!! Form::select('npk_pic', \DB::table("mgmt_pics")->selectRaw("npk, npk||' - '||coalesce((select coalesce(v.initial, split_part(v.nama,' ',1)) from v_mas_karyawan v where v.npk = mgmt_pics.npk limit 1),'-') initial")->where('st_aktif', 'T')->where('st_bod', 'T')->where('kd_site', 'IGPJ')->orderBy('npk')->pluck('initial','npk')->all(), null, ['class'=>'form-control select2', 'placeholder' => 'Pilih PIC Genba', 'required']) !!}
        ...
        <div class="col-sm-5 {{ $errors->has('cm_pict') ? ' has-error' : '' }}">
          ...
          @if (!empty($mgmtgemba->cm_pict))
            <p>
              ...
              <a class="btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Hapus File" href="{{ route('mgmtgembas.deleteimage', [base64_encode($mgmtgemba->no_gemba), base64_encode("CM")]) }}"><span class="glyphicon glyphicon-remove"></span></a>
            </p>
          @endif
...
<div class="box-footer">
  {!! Form::submit('Save Genba BOD', ['class'=>'btn btn-primary', 'id' => 'btn-save']) !!}
  @if (!empty($mgmtgemba->no_gemba) && $mode_cm === "F")
    ...
    <a class="btn btn-success" href="{{ route('mgmtgembas.create') }}">
      <span class="fa fa-plus"></span> Add Genba BOD
    </a>
  @endif
  &nbsp;&nbsp;
  <a class="btn btn-default" href="{{ route('mgmtgembas.index') }}">Cancel</a>
...
<script type="text/javascript">
  ...
  function changeKdSite() {
    ...
    if(kd_site === "IGPK") {
      ...
      @foreach(\DB::table("mgmt_pics")->selectRaw("npk, npk||' - '||coalesce((select coalesce(v.initial, split_part(v.nama,' ',1)) from v_mas_karyawan v where v.npk = mgmt_pics.npk limit 1),'-') initial")->where('st_aktif', 'T')->where('st_bod', 'T')->where('kd_site', 'IGPK')->orderBy('npk')->get() as $mgmt_pic)  
        $("#npk_pic").append('<option value="{{ $mgmt_pic->npk }}">{{ $mgmt_pic->initial }}</option>');
      @endforeach
      ...
      @foreach(\DB::table("mgmt_pics")->selectRaw("npk, npk||' - '||coalesce((select coalesce(v.initial, split_part(v.nama,' ',1)) from v_mas_karyawan v where v.npk = mgmt_pics.npk limit 1),'-') initial")->where('st_aktif', 'T')->where('st_bod', 'T')->where('kd_site', 'IGPJ')->orderBy('npk')->get() as $mgmt_pic)  
        $("#npk_pic").append('<option value="{{ $mgmt_pic->npk }}">{{ $mgmt_pic->initial }}</option>');
      @endforeach
    }
