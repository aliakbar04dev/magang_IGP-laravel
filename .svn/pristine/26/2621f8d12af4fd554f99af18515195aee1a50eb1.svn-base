...
@section('content')
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        CR Activities [Dept]
        <small>Detail CR Activities [Approval Dept. Head]</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="{{ url('/home') }}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><i class="fa fa-files-o"></i> Budget - TRANSAKSI</li>
        <li><a href="{{ route('bgttcrregiss.indexdep') }}"><i class="fa fa-files-o"></i> CR Activities</a></li>
    ...
    <section class="content">
      ...
      <div class="box-footer">
        @if ($bgttcrregis->checkKdDept() === "T" && $bgttcrregis->status === "SUBMIT")
          @if (Auth::user()->can('budget-cr-activities-approve-dep'))
            <button id='btnapprove' type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Approve Activities - Dept. Head' onclick='approve()'>
              <span class='glyphicon glyphicon-check'></span> Approve Dept. Head
            </button>
            &nbsp;&nbsp;
            <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject Activities - Dept. Head' onclick='reject()'>
              <span class='glyphicon glyphicon-remove'></span> Reject Dept. Head
            </button>
            &nbsp;&nbsp;
          @endif
        @endif
        <a class="btn btn-primary" href="{{ route('bgttcrregiss.indexdep') }}" data-toggle="tooltip" data-placement="top" title="Kembali ke Daftar CR Activities">Cancel</a>
      </div>
    </section>
    <!-- /.content -->
  </div>
...
<script type="text/javascript">
  ...
  function approve()
  {
    ...
    swal({
      ...
      var url = "{{ route('bgttcrregiss.approvedep')}}";
      $("#loading").show();
      $.ajax({
        type     : 'POST',
        url      : url,
        ...
        success:function(data){
          ...
          if(data.status === 'OK'){
            ...
            var urlRedirect = "{{ route('bgttcrregiss.showdep', 'param') }}";
  ...
  function reject()
  {
    ...
    swal({
      ...
      swal({
        ...
        // save via ajax
        // create data detail dengan ajax
        var url = "{{ route('bgttcrregiss.rejectdep')}}";
        $("#loading").show();
        $.ajax({
          type     : 'POST',
          url      : url,
