    ...
    <div class="box-body">
      <div class="row form-group">
        ...
        <div class="col-sm-2 {{ $errors->has('nil_diamet_mm') ? ' has-error' : '' }}">
          ...
          {!! Form::number('nil_diamet_mm', !empty($prctrfq->prctRfqRm()) ? numberFormatterForm(0, 2)->format($prctrfq->prctRfqRm()->nil_diamet_mm) : null, ['class'=>'form-control', 'placeholder' => 'Diameter (mm)', 'max' => 9999999999.99, 'step' => 'any', 'id' => 'nil_diamet_mm', 'onchange' => 'refreshTotal()', 'style' => 'text-align:right;']) !!}
        ...
        <div class="col-sm-2 {{ $errors->has('nil_length_mm') ? ' has-error' : '' }}">
          ...
          {!! Form::number('nil_length_mm', !empty($prctrfq->prctRfqRm()) ? numberFormatterForm(0, 2)->format($prctrfq->prctRfqRm()->nil_length_mm) : null, ['class'=>'form-control', 'placeholder' => 'Length (mm)', 'max' => 9999999999.99, 'step' => 'any', 'id' => 'nil_length_mm', 'onchange' => 'refreshTotal()', 'style' => 'text-align:right;']) !!}
      ...
      <div class="row form-group">
        ...
        <div class="col-sm-2 {{ $errors->has('out_weight_kg') ? ' has-error' : '' }}">
          ...
          {!! Form::number('out_weight_kg', !empty($prctrfq->prctRfqRm()) ? numberFormatterForm(0, 2)->format($prctrfq->prctRfqRm()->out_weight_kg) : null, ['class'=>'form-control', 'placeholder' => 'Output Weight (KG)', 'max' => 9999999999.99, 'step' => 'any', 'id' => 'out_weight_kg', 'style' => 'text-align:right;', 'onchange' => 'refreshTotal()']) !!}
        ...
        <div class="col-sm-2 {{ $errors->has('scrap_pric') ? ' has-error' : '' }}">
          ...
          {!! Form::number('scrap_pric', !empty($prctrfq->prctRfqRm()) ? numberFormatterForm(0, 2)->format($prctrfq->prctRfqRm()->scrap_pric) : null, ['class'=>'form-control', 'placeholder' => 'Scrap Price (IDR)', 'max' => 9999999999.99, 'step' => 'any', 'id' => 'scrap_pric', 'onchange' => 'refreshTotal()', 'style' => 'text-align:right;']) !!}
    ...
    <div class="box-body" id="field-proses">
      ...
      @foreach ($prctrfq->prctRfqProsess() as $model)
        <div class="row form-group" id="proses_field_{{ $loop->iteration }}">
          ...
          <div class="col-sm-4">
            <input type="text" id="proses_mesin_type_{{ $loop->iteration }}" name="proses_mesin_type_{{ $loop->iteration }}" required class="form-control" placeholder="Machine Type/ Spec" maxlength="50" value="{{ $model->mesin_type }}">
          </div>
          <div class="col-sm-2">
  ...
  <div role="tabpanel" class="tab-pane" id="ht">
    <div class="box-body" id="field-ht">
      <div class="row form-group">
        ...
        <div class="col-sm-1">
          <label name="ht_btndelete">Action</label>
        </div>
      </div>
      <!-- /.form-group -->
      @foreach ($prctrfq->prctRfqHts() as $model)
        <div class="row form-group" id="ht_field_{{ $loop->iteration }}">
          <div class="col-sm-2">
            <input type="number" id="ht_no_urut_{{ $loop->iteration }}" name="ht_no_urut_{{ $loop->iteration }}" required class="form-control" placeholder="No. Urut" min="1" max="9999999999" value="{{ $model->no_urut }}">
          ...
          <div class="col-sm-5">
            <input type="text" id="ht_nm_ht_{{ $loop->iteration }}" name="ht_nm_ht_{{ $loop->iteration }}" required class="form-control" placeholder="Nama HT" maxlength="50" value="{{ $model->nm_ht }}">
          </div>
          <div class="col-sm-2">
            <input type="number" id="ht_rate_per_kg_{{ $loop->iteration }}" name="ht_rate_per_kg_{{ $loop->iteration }}" class="form-control" placeholder="Rate/Kg (IDR)" max="9999999999.99" step="any" value="{{ numberFormatterForm(0, 2)->format($model->rate_per_kg) }}" onchange="refreshTotal()" style="text-align:right;">
          </div>
          <div class="col-sm-2">
          ...
          <div class="col-sm-1">
            <button id="ht_btndelete_{{ $loop->iteration }}" name="ht_btndelete_{{ $loop->iteration }}" type="button" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="top" title="Hapus HT" onclick="deleteHt(this)">
              <i class="fa fa-times"></i>
            </button>
          </div>
        </div>
    ...
    <div class="box-body">
      <p class="pull-right">
      <button id="addRowHt" type="button" class="btn btn-success" data-toggle="tooltip" data-placement="top" title="Add HT"><span class="glyphicon glyphicon-plus"></span> Add HT</button>
      </p>
    </div>
    <!-- /.box-body -->
    <div class="box-body">
      ...
      @foreach ($prctrfq->prctRfqTools() as $model)
        <div class="row form-group" id="tool_field_{{ $loop->iteration }}">
          ...
          <div class="col-sm-2">
            ...
            <input type="number" id="tool_life_time_{{ $loop->iteration }}" name="tool_life_time_{{ $loop->iteration }}" class="form-control" placeholder="Lifetime Dies" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;" value="{{ numberFormatterForm(0, 2)->format($model->life_time) }}">
          </div>
          <div class="col-sm-2">
...
<script type="text/javascript">
  ...
  function refreshTab() {
    ...
    var st_ht = document.getElementById("st_ht").value.trim();
  ...
  $("#addRowProses").click(function(){
    ...
    $("#field-proses").append(
      '<div class="row form-group" id="' + proses_id_field + '">\
        ...
        <div class="col-sm-4">\
          <input type="text" id="' + proses_mesin_type + '" name="' + proses_mesin_type + '" required class="form-control" placeholder="Machine Type/ Spec" maxlength="50">\
        </div>\
        <div class="col-sm-2">\
  ...
  $("#addRowHt").click(function(){
    var jml_row_ht = document.getElementById("jml_row_ht").value.trim();
    jml_row_ht = Number(jml_row_ht) + 1;
    document.getElementById("jml_row_ht").value = jml_row_ht;

    var ht_no_ht = 'ht_no_ht_'+jml_row_ht;
    var ht_no_urut = 'ht_no_urut_'+jml_row_ht;
    var ht_nm_ht = 'ht_nm_ht_'+jml_row_ht;
    var ht_rate_per_kg = 'ht_rate_per_kg_'+jml_row_ht;
    var ht_ttl_ht = 'ht_ttl_ht_'+jml_row_ht;
    var ht_btndelete = 'ht_btndelete_'+jml_row_ht;
    var ht_id_field = 'ht_field_'+jml_row_ht;
    var ht_id_hr = 'ht_hr_'+jml_row_ht;

    $("#field-ht").append(
      '<div class="row form-group" id="' + ht_id_field + '">\
        <div class="col-sm-2">\
          <input type="number" id="' + ht_no_urut + '" name="' + ht_no_urut + '" required class="form-control" placeholder="No. Urut" min="1" max="9999999999">\
          <input type="hidden" id="' + ht_no_ht + '" name="' + ht_no_ht + '" class="form-control" readonly="readonly">\
        </div>\
        <div class="col-sm-5">\
          <input type="text" id="' + ht_nm_ht + '" name="' + ht_nm_ht + '" required class="form-control" placeholder="Nama HT" maxlength="50">\
        </div>\
        <div class="col-sm-2">\
          <input type="number" id="' + ht_rate_per_kg + '" name="' + ht_rate_per_kg + '" class="form-control" placeholder="Rate/Kg (IDR)" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;">\
        </div>\
        <div class="col-sm-2">\
          <input type="text" id="' + ht_ttl_ht + '" name="' + ht_ttl_ht + '" class="form-control" placeholder="Total (IDR)" disabled="" style="text-align:right;">\
        </div>\
        <div class="col-sm-1">\
          <button id="' + ht_btndelete + '" name="' + ht_btndelete + '" type="button" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="top" title="Hapus HT" onclick="deleteHt(this)">\
            <i class="fa fa-times"></i>\
          </button>\
        </div>\
      </div>\
    ...
    document.getElementById(ht_no_urut).focus();
  });

  function changeIdHt(row) {
    var id_field = "#ht_field_" + row;
    $(id_field).remove();
    var id_hr = "#ht_hr_" + row;
    $(id_hr).remove();

    var jml_row_ht = document.getElementById("jml_row_ht").value.trim();
    jml_row_ht = Number(jml_row_ht);
    nextRow = Number(row) + 1;
    for($i = nextRow; $i <= jml_row_ht; $i++) {
      var id_no_ht = "#ht_no_ht_" + $i;
      var id_no_ht_new = "ht_no_ht_" + ($i-1);
      $(id_no_ht).attr({"id":id_no_ht_new, "name":id_no_ht_new});
      var id_no_urut = "#ht_no_urut_" + $i;
      var id_no_urut_new = "ht_no_urut_" + ($i-1);
      $(id_no_urut).attr({"id":id_no_urut_new, "name":id_no_urut_new});
      var id_nm_ht = "#ht_nm_ht_" + $i;
      var id_nm_ht_new = "ht_nm_ht_" + ($i-1);
      $(id_nm_ht).attr({"id":id_nm_ht_new, "name":id_nm_ht_new});
      var id_rate_per_kg = "#ht_rate_per_kg_" + $i;
      var id_rate_per_kg_new = "ht_rate_per_kg_" + ($i-1);
      $(id_rate_per_kg).attr({"id":id_rate_per_kg_new, "name":id_rate_per_kg_new});
      var id_ttl_ht = "#ht_ttl_ht_" + $i;
      var id_ttl_ht_new = "ht_ttl_ht_" + ($i-1);
      $(id_ttl_ht).attr({"id":id_ttl_ht_new, "name":id_ttl_ht_new});
      var btndelete = "#ht_btndelete_" + $i;
      var btndelete_new = "ht_btndelete_" + ($i-1);
      $(btndelete).attr({"id":btndelete_new, "name":btndelete_new});
      var id_field = "#ht_field_" + $i;
      var id_field_new = "ht_field_" + ($i-1);
      $(id_field).attr({"id":id_field_new, "name":id_field_new});
      var id_hr = "#ht_hr_" + $i;
      var id_hr_new = "ht_hr_" + ($i-1);
      $(id_hr).attr({"id":id_hr_new, "name":id_hr_new});
    }
    jml_row_ht = jml_row_ht - 1;
    document.getElementById("jml_row_ht").value = jml_row_ht;

    refreshTotal();
  }

  function deleteHt(ths) {
    var msg = 'Anda yakin menghapus HT ini?';
    swal({
      title: msg,
      text: "",
      type: 'warning',
      ...
      //startcode
      var row = ths.id.replace('ht_btndelete_', '');
      var info = "";
      var info2 = "";
      var info3 = "warning";
      var no_rfq = document.getElementById("no_rfq").value.trim();
      var no_rev = document.getElementById("no_rev").value.trim();
      var no_ht = document.getElementById("ht_no_ht_" + row).value.trim();      
      if(no_ht === "") {
        changeIdHt(row);
      } else {
        //DELETE DI DATABASE
        ...
        // delete via ajax
        // hapus data detail dengan ajax
        var url = "{{ route('prctrfqs.deleteht', ['param','param2','param3']) }}";
        url = url.replace('param3', window.btoa(no_ht));
        url = url.replace('param2', window.btoa(no_rev));
        url = url.replace('param', window.btoa(no_rfq));
        $.ajax({
          type     : 'POST',
  ...
  $("#addRowTool").click(function(){
    ...
    $("#field-tool").append(
      '<div class="row form-group" id="' + tool_id_field + '">\
        ...
        <div class="col-sm-2">\
          <label name="' + tool_life_time + '">Lifetime Dies</label>\
          <input type="number" id="' + tool_life_time + '" name="' + tool_life_time + '" class="form-control" placeholder="Lifetime Dies" max="9999999999.99" step="any" onchange="refreshTotal()" style="text-align:right;">\
        </div>\
        <div class="col-sm-2">\
