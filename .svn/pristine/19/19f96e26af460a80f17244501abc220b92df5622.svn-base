...
@section('content')
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        CR Progress Report
        <small>CR Progress Report</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="{{ url('/home') }}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><i class="fa fa-files-o"></i> Budget - REPORT</li>
        <li class="active"><i class="fa fa-files-o"></i> CR Progress Report</li>
      </ol>
    </section>
        ...
        <div class="col-md-12">
          <div class="box box-primary">
            <div class="box-body form-horizontal">
              ...
              <div class="form-group">
                <div class="col-sm-6">
                  {!! Form::label('lbldiv', 'Division') !!}
                  <select name="filter_div" aria-controls="filter_status" class="form-control select2" multiple="multiple">
                    @foreach($vw_dep_budgets->get() as $kode)
                      <option value="{{$kode->kd_div}}">{{$kode->desc_div}}</option>
                    @endforeach
                  </select>
                </div>
                <div class="col-sm-2">
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    var tableMaster = $('#tblMaster').DataTable({
      ...
      // serverSide: true,
      ajax: "{{ route('bgttcrsubmits.dashboardreport') }}",
      columns: [
      	{data: null, name: null},
    ...
    $('#tblMaster tbody').on( 'click', 'tr', function () {
      if ($(this).hasClass('selected') ) {
        ...
        if(tableMaster.rows().count() > 0) {
          ...
          var kd_div = tableMaster.cell(index, 1).data();
          var info = "Division: " + kd_div + ", Tahun: " + tahun + ", Bulan: " + bulan;
          document.getElementById("info-detail").innerHTML = 'Detail (' + info + ')';
          var regex = /(<([^>]+)>)/ig;
          initTable(window.btoa(tahun.replace(regex, "")), window.btoa(bulan.replace(regex, "")), window.btoa(kd_div.replace(regex, "")));
        }
      }
    });

    var url = '{{ route('bgttcrsubmits.dashboardreportdetail', ['param', 'param2', 'param3']) }}';
    ...
    function initTable(tahun, bulan, kd_div) {
      tableDetail.search('').columns().search('').draw();
      var url = '{{ route('bgttcrsubmits.dashboardreportdetail', ['param', 'param2', 'param3']) }}';
      url = url.replace('param3', kd_div);
    ...
    $("#tblMaster").on('preXhr.dt', function(e, settings, data) {
      ...
      data.kd_div = $('select[name="filter_div"]').val();
