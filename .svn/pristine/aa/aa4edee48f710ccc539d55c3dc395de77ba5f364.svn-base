  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{ route('hrdtidpdep1s.approvalhrd') }}"><i class="fa fa-files-o"></i> Daftar IDP</a></li>
      ...
      {!! Form::model($hrdtidpdep1, ['url' => route('hrdtidpdep1s.update', base64_encode($hrdtidpdep1->id)),
            ...
            <div class="box box-primary">
                        ...
                        @if ($hrdtidpdep1->hrdtIdpdep1Rejects()->get()->count() > 0)
                          ...
                          @foreach ($hrdtidpdep1->hrdtIdpdep1Rejects()->get() as $hrdtidpdep1Reject)
                            @if (Auth::user()->can(['hrd-idpdep-approve-hrd','hrd-idpdep-reject-hrd']))
                              {{ "&nbsp;-&nbsp;&nbsp;" }}<a target="_blank" href="{{ route('hrdtidpdep1s.showapprovalhrdrevisi', base64_encode($hrdtidpdep1Reject->id)) }}" data-toggle="tooltip" data-placement="top" title="Show History IDP No. Revisi {{ $hrdtidpdep1Reject->revisi }}">{{ $hrdtidpdep1Reject->revisi }}</a>
                            @else
              ...
              @if ($hrdtidpdep1->hrdtIdpdep2sByStatus("W")->get()->count() > 0)
                <div class="box-body">
                  <div class="row">
                    <div class="col-md-12">
                      <div class="box box-primary{{ $hrdtidpdep1->status !== "SUBMIT" ? " collapsed-box" : "" }}">
                        <div class="box-header with-border">
              ...
              @if ($hrdtidpdep1->hrdtIdpdep4s()->get()->count() > 0)
                <div class="box-body">
                  <div class="row">
                    <div class="col-md-12">
                      <div class="box box-primary{{ $hrdtidpdep1->status !== "SUBMIT (MID)" ? " collapsed-box" : "" }}">
                        <div class="box-header with-border">
              ...
              @if ($hrdtidpdep1->hrdtIdpdep5s()->get()->count() > 0)
                <div class="box-body">
                  <div class="row">
                    <div class="col-md-12">
                      <div class="box box-primary{{ $hrdtidpdep1->status !== "SUBMIT (ONE)" ? " collapsed-box" : "" }}">
                        <div class="box-header with-border">
        ...
        <div class="box-footer">
          @if ($hrdtidpdep1->status === "SUBMIT" || $hrdtidpdep1->status === "SUBMIT (MID)" || $hrdtidpdep1->status === "SUBMIT (ONE)")
            @if (Auth::user()->can(['hrd-idpdep-approve-hrd','hrd-idpdep-reject-hrd']))
              @if ($hrdtidpdep1->status === "SUBMIT")
                @if (Auth::user()->can('hrd-idpdep-approve-hrd'))
                  {!! Form::submit('Approve HRD', ['class'=>'btn btn-primary', 'id' => 'btn-save']) !!}
                  &nbsp;&nbsp;
                @endif
                @if (Auth::user()->can('hrd-idpdep-reject-hrd'))
                  <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject HRD' onclick='reject("{{ base64_encode($hrdtidpdep1->id) }}")'>
                    <span class='glyphicon glyphicon-remove'></span> Reject HRD
                  </button>
                  &nbsp;&nbsp;
                @endif
              @elseif ($hrdtidpdep1->status === "SUBMIT (MID)")
                @if (Auth::user()->can('hrd-idpdep-approve-hrd'))
                  {!! Form::submit('Approve HRD (MID)', ['class'=>'btn btn-primary', 'id' => 'btn-save']) !!}
                  &nbsp;&nbsp;
                @endif
                @if (Auth::user()->can('hrd-idpdep-reject-hrd'))
                  <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject HRD (MID) IDP' onclick='reject("{{ base64_encode($hrdtidpdep1->id) }}")'>
                    <span class='glyphicon glyphicon-remove'></span> Reject HRD (MID)
                  </button>
                  &nbsp;&nbsp;
                @endif
              @elseif ($hrdtidpdep1->status === "SUBMIT (ONE)")
                @if (Auth::user()->can('hrd-idpdep-approve-hrd'))
                  {!! Form::submit('Approve HRD (ONE)', ['class'=>'btn btn-primary', 'id' => 'btn-save']) !!}
                  &nbsp;&nbsp;
                @endif
                @if (Auth::user()->can('hrd-idpdep-reject-hrd'))
                  <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject HRD (ONE)' onclick='reject("{{ base64_encode($hrdtidpdep1->id) }}")'>
                    <span class='glyphicon glyphicon-remove'></span> Reject HRD (ONE)
                  </button>
                  &nbsp;&nbsp;
                @endif
              @endif
          ...
          &nbsp;&nbsp;
          <a class="btn btn-primary" href="{{ route('hrdtidpdep1s.approvalhrd') }}" data-toggle="tooltip" data-placement="top" title="Kembali ke Dashboard">Cancel</a>
        </div>
      {!! Form::close() !!}
    </section>
    <!-- /.content -->
    ...
    if (!localParams.send) {
      ...
      if(valid_data === "F") {
        ...
        swal({
          ...
          document.getElementById("st_input").value = "HRD";
  ...
  function reject(id)
  {
    ...
    swal({
      ...
      swal({
        ...
        var urlRedirect = "{{ route('hrdtidpdep1s.reject', ['param', 'param2', 'param3']) }}";
        urlRedirect = urlRedirect.replace('param3', window.btoa("HRD"));
