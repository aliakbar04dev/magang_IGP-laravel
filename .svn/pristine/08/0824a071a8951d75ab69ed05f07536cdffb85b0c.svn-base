...
<p>
	Telah disetujui KPI Division {{ $hrdtkpi->desc_div }} oleh Superior: {{ Auth::user()->name }} ({{ Auth::user()->username }}) pada tanggal {{ \Carbon\Carbon::now()->format('d M Y H:i') }} dengan detail sebagai berikut:
</p>
