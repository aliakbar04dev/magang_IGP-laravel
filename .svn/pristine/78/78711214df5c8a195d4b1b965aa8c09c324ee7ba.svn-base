...
<p>
	Kepada: Bapak/Ibu <strong>{{ $hrdtidp1->namaByNpk($hrdtidp1->npk_div_head) }}</strong>
</p>

<p>
	Telah disubmit Individual Development Plan Section Head oleh {{ Auth::user()->name }} ({{ Auth::user()->username }}) pada tanggal {{ \Carbon\Carbon::now()->format('d M Y H:i') }} dengan detail sebagai berikut:
</p>
