  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        ...
        <small>Detail CR Activities Progress [Div]</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="{{ url('/home') }}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><i class="fa fa-files-o"></i> Budget - TRANSAKSI</li>
        <li><a href="{{ route('bgttcrsubmits.indexdiv') }}"><i class="fa fa-files-o"></i> CR Activities Progress [Div]</a></li>
    ...
    <section class="content">
        ...
        <div class="col-md-12">
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">CR Activities Progress [Div]</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                  ...
                  <tr>
                    ...
                    <td style="width: 25%;">
                      @if (Auth::user()->can('budget-cr-activities-approve-div'))
                        <a target="_blank" href="{{ route('bgttcrregiss.showdiv', base64_encode($bgttcrsubmit->id_regis)) }}" data-toggle="tooltip" data-placement="top" title="Show Detail Register">
      ...
      <div class="box-footer">
        @if ($bgttcrsubmit->checkKdDiv() === "T" && $bgttcrsubmit->status === "APPROVE DEPT")
          @if (Auth::user()->can('budget-cr-activities-approve-div'))
            <button id='btnapprove' type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Approve Activities Progress - Div. Head' onclick='approve()'>
              <span class='glyphicon glyphicon-check'></span> Approve Div. Head
            </button>
            &nbsp;&nbsp;
            <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject Activities Progress - Div. Head' onclick='reject()'>
              <span class='glyphicon glyphicon-remove'></span> Reject Div. Head
            </button>
            &nbsp;&nbsp;
          @endif
        @endif
        <a class="btn btn-primary" href="{{ route('bgttcrsubmits.indexdiv') }}" data-toggle="tooltip" data-placement="top" title="Kembali ke Daftar CR Activities Progress">Cancel</a>
      </div>
    </section>
    <!-- /.content -->
  </div>
    ...
    swal({
      ...
      $.ajax({
        ...
        data     : {
          ...
          _token         : token,
          ids            : "{{ $bgttcrsubmit->id }}",
          status_approve : window.btoa("DIV")
        },
        success:function(data){
          $("#loading").hide();
          if(data.status === 'OK'){
            swal("Approved", data.message, "success");
            var urlRedirect = "{{ route('bgttcrsubmits.showdiv', 'param') }}";
