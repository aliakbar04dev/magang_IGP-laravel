<div class="box-body">
  <div class="form-group">
    <div class="col-sm-3 {{ $errors->has('no_lfk') ? ' has-error' : '' }}">
      {!! Form::hidden('st_serah', "LFK", ['class' => 'form-control', 'readonly' => 'readonly', 'id' => 'st_serah']) !!}
      {!! Form::hidden('vouchers', null, ['class' => 'form-control', 'readonly' => 'readonly', 'id' => 'vouchers']) !!}
      {!! Form::label('no_lfk', 'No. Serah Terima (*)') !!}
      {!! Form::text('no_lfk', null, ['class' => 'form-control', 'placeholder' => 'No. Serah Terima', 'readonly' => 'readonly']) !!}
      {!! $errors->first('no_lfk', '<p class="help-block">:message</p>') !!}
    </div>
    <div class="col-sm-3 {{ $errors->has('tgl_lfk') ? ' has-error' : '' }}">
      {!! Form::label('tgl_lfk', 'Tanggal Serah (*)') !!}
      @if (empty($model->tgl_lfk))
        {!! Form::date('tgl_lfk', \Carbon\Carbon::now(), ['class' => 'form-control', 'placeholder' => 'Tanggal Serah', 'disabled' => '']) !!}
      @else
        {!! Form::date('tgl_lfk', \Carbon\Carbon::parse($model->tgl_lfk), ['class' => 'form-control','placeholder' => 'Tanggal Serah', 'disabled' => '']) !!}
      @endif
      {!! $errors->first('tgl_lfk', '<p class="help-block">:message</p>') !!}
    </div>
    @if (!empty($model->no_lfk))
      <div class="col-sm-4">
        {!! Form::label('info', 'Total Serah ke KASIR/Total Voucher Dari Kasir') !!}
        {!! Form::text('info', $info, ['class' => 'form-control', 'placeholder' => 'Total Serah ke KASIR/Total Voucher Dari Kasir', 'disabled' => '']) !!}
      </div>
    @endif
  </div>
  <!-- /.form-group -->
  <div class="form-group">
    <div class="col-sm-6 {{ $errors->has('ket_lfk') ? ' has-error' : '' }}">
      {!! Form::label('ket_lfk', 'Keterangan') !!}
      @if (empty($model->no_lfk))
        {!! Form::text('ket_lfk', 'FINANCE KE KASIR', ['class' => 'form-control', 'placeholder' => 'Keterangan', 'maxlength' => 50]) !!}
      @else
        @if($jml_tarik > 0)
          {!! Form::text('ket_lfk', null, ['class' => 'form-control', 'placeholder' => 'Keterangan', 'maxlength' => 50, 'readonly' => 'readonly']) !!}
        @else 
          {!! Form::text('ket_lfk', null, ['class' => 'form-control', 'placeholder' => 'Keterangan', 'maxlength' => 50]) !!}
        @endif
      @endif
      {!! $errors->first('ket_lfk', '<p class="help-block">:message</p>') !!}
    </div>
    <div class="col-sm-2">
...
<div class="box-footer">
  ...
  @endif
  &nbsp;&nbsp;
  <a class="btn bg-black" href="{{ route('lalins.indexfinksr') }}">Cancel</a>
...
<script type="text/javascript">
  ...
  document.getElementById("ket_lfk").focus();

  //Initialize Select2 Elements
  $(".select2").select2();

  $("#btn-delete").click(function(){
    var no_lfk = document.getElementById("no_lfk").value.trim();
    var msg = 'Anda yakin menghapus No. Serah Terima: ' + no_lfk + '?';
    var txt = '';
    swal({
      title: msg,
      text: txt,
      ...
      urlRedirect = urlRedirect.replace('param', window.btoa(no_lfk));
    ...
    if (!localParams.send) {
      ...
      if(valid !== "T") {
        ...
        swal({
          ...
          document.getElementById("st_serah").value = "LFK";
  ...
  $("#btn-serah").click(function(){
    ...
    var url = '{{ route('lalins.popupserahfinksr') }}';
  ...
  $("#btn-pilih").click(function(){
    ...
    if(validasi === "T") {
      ...
      url = url.replace('param3', window.btoa(no_lfk));
  ...
  $(document).ready(function(){
    ...
    var url = '{{ route('lalins.detailfinksr', 'param') }}';
    url = url.replace('param', window.btoa(no_lfk));
    var tableVoucher = $('#tblVoucher').DataTable({
      "columnDefs": [{
        "searchable": false,
        "orderable": false,
  ...
  function deleteDetail(no_lfk, no_voucher) {
