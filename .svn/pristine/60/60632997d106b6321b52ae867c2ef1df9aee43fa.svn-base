  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{ route('prctrfqs.indexanalisa') }}"><i class="fa fa-files-o"></i> Analisa RFQ</a></li>
        <li class="active">Detail RFQ: {{ $prctrfq->no_rfq }}</li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      @if (strlen(Auth::user()->username) == 5)
        <div class="box-footer">
          @if ($prctrfq->tgl_supp_submit != null && $prctrfq->tgl_rjt_prc == null && $prctrfq->tgl_close == null)
            @if ($prctrfq->tgl_apr_prc != null)
              <button id='btnreject' type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Reject RFQ {{ $prctrfq->no_rfq }}' onclick='rejectRfq("{{ $prctrfq->no_rfq }}", "PRC")'>
                <span class='glyphicon glyphicon-remove'></span> Reject RFQ
              </button>
            @else
              <button id='btnapprove' type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Approve RFQ {{ $prctrfq->no_rfq }}' onclick='approveRfq("{{ $prctrfq->no_rfq }}", "PRC")'>
