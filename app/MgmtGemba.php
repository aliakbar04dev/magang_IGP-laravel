...
class MgmtGemba extends Model
{
    ...
    protected $fillable = [
    	'no_gemba', 'tgl_gemba', 'kd_site', 'pict_gemba', 'det_gemba', 'kd_area', 'lokasi', 'npk_pic', 'cm_pict', 'cm_ket', 'st_gemba', 'dtcrea', 'creaby', 'dtmodi', 'modiby', 'dep_gemba'
    ];
        ...
        self::deleting(function($mgmtgemba) {
            ...
            if($mgmtgemba->dep_gemba === "BOD") {
                if(!Auth::user()->can('mgt-gemba-delete')) {
                    ...
                    } else if(!Auth::user()->can('mgt-gemba-site')) {
	...
	public function checkEdit() {
		$valid = "T";
        if($this->dep_gemba !== "BOD") {
            $valid = "F";
        } else if(!Auth::user()->can('mgt-gemba-create')) {
			$valid = "F";
		} else {
            // if($this->st_gemba === "T") {
            //     $valid = "F";
    ...
    public function getInitialAttribute()
    {
        ...
        $initial = DB::table("v_mas_karyawan")
        ->selectRaw("coalesce(initial, split_part(nama,' ',1)) initial")
        ->where('npk', $npk_pic)
        ->value("initial");
        if($initial == null) {
            $initial = "-";
