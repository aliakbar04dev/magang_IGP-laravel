...
class HrdtIdp1 extends Model
{
    ...
    public static function boot()
	{
		...
		self::deleting(function($hrdtidp1) {
			if($hrdtidp1->npk_dep_head !== Auth::user()->username) {
                ...
                return false;
			} else if(strtoupper($hrdtidp1->status) !== 'DRAFT') {
                Session::flash("flash_notification", [
                    "level"=>"danger",
                    "message"=>"IDP $hrdtidp1->tahun gagal dihapus karena sudah di-$hrdtidp1->status."
                ]);
                // membatalkan proses penghapusan
                return false;
            }
    ...
    public function hrdtIdp1Rejects() {
        $lists = DB::table("hrdt_idp1_rejects")
            ->select(DB::raw("*"))
            ->where("npk", $this->npk)
	...
	public function hrdtIdp2s() {
		return $this->hasMany('App\HrdtIdp2');
	}

    public function hrdtIdp4s() {
        return $this->hasMany('App\HrdtIdp4');
    }

    public function hrdtIdp5s() {
        return $this->hasMany('App\HrdtIdp5');
    }
    ...
    public function checkEdit() {
	    $valid = "T";
	    if(strtoupper($this->status) === 'SUBMIT' || strtoupper($this->status) === 'REJECT' || strtoupper($this->status) === 'APPROVE DIVISI' || strtoupper($this->status) === 'SUBMIT (MID)' || strtoupper($this->status) === 'REJECT (MID)' || strtoupper($this->status) === 'APPROVE DIVISI (MID)' || strtoupper($this->status) === 'SUBMIT (ONE)' || strtoupper($this->status) === 'REJECT (ONE)' || strtoupper($this->status) === 'APPROVE DIVISI (ONE)' || strtoupper($this->status) === 'APPROVE HRD (ONE)') {
            if(strtoupper($this->status) === 'SUBMIT' || strtoupper($this->status) === 'SUBMIT (MID)' || strtoupper($this->status) === 'SUBMIT (ONE)') {
                if(!Auth::user()->can('hrd-idp-approve-div') || $this->npk_div_head !== Auth::user()->username) {
                    $valid = "F";
                }
            } else {
                $valid = "F";
            }
	    } else if($this->npk_dep_head !== Auth::user()->username) {
            $valid = "F";
        }
	    return $valid;
    }
