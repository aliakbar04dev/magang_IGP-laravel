...
class User extends Authenticatable
{
    ...
    public function emailByUsername($username)
    {
    ...
    public function fotoKaryawan() {
        if(config('app.env', 'local') === 'production') {
            $file_temp = DIRECTORY_SEPARATOR."serverh".DIRECTORY_SEPARATOR."Portal".DIRECTORY_SEPARATOR."foto".DIRECTORY_SEPARATOR.$this->username.".jpg";
        } else {
            $file_temp = "\\\\".config('app.ip_x', '-')."\\Batch\\Hrms_new\\foto\\".$this->username.".jpg";
        }
        if (!file_exists($file_temp)) {
            if($this->masKaryawan() != null) {
                if(config('app.env', 'local') === 'production') {
                    $file_temp = DIRECTORY_SEPARATOR."serverh".DIRECTORY_SEPARATOR."Portal".DIRECTORY_SEPARATOR."foto".DIRECTORY_SEPARATOR.$this->masKaryawan()->foto;
                } else {
                    $file_temp = "\\\\".config('app.ip_x', '-')."\\Batch\\Hrms_new\\foto\\".$this->masKaryawan()->foto;
                }
            }
        }
        if (file_exists($file_temp)) {
